...
class Linear:
    ...
    def __init__(self, in_layer, num_out_features):
        ...
        # self.W = np.random.randn(num_out_features, self.num_in_features) * 0.01  # col = no of features
        self.W = np.random.randn(num_out_features, self.num_in_features)  # col = no of features
    ...
    def backward(self, dwnstrm):
        ...
        # self.G = self.in_array * dwnstrm # 1a
...
class Bias:
    ...
    def __init__(self, in_layer):
        ...
        # self.W = np.random.rand(1, self.num_in_features)
...
class SGDSolver:
    ...
    def step(self):
        ...
        for m in self.modules:
            ...
            if (dbg): print('shape of weight = ', m.W.shape)
            if (dbg): print('shape of gradient = ', m.G.shape)
            if (dbg): print('Weight vector-')
            if (dbg): print(m.W)
            if (dbg): print('Gradient vector')
            if (dbg): print(m.G)
            if (dbg): print('To substract from weight vector')
            # if (dbg): print(np.sum(m.G, axis = 0))
            ...
            if (m.G.shape == m.W.shape):
                ...
                if (dbg): print(self.lr*np.sum(m.G, axis = 0))
                m.W -= (self.lr*(np.sum(m.G, axis = 0)))
